...
shopSenseApp.controller('CrudCtrl', ['$scope', '$firebase', 
  function($scope, $firebase) {
    ...
    $scope.posts = sync.$asArray();
  	...
  	$scope.addPost = function(e) {
        ...
        if (e.keyCode === 13 && $scope.posts) {
        	...
        	$scope.authorInput = "";
        	$scope.titleInput = "";
        	// SHOW THE POST OBJECT IN CONSOLE.
	...
	postRef.orderByChild("author").on("child_added", function(snapshot) {
	  console.log(snapshot.key() + " was written by " + snapshot.val().author + " and they are great");
	});

// NOTES FROM FIREANGULAR ON HOW TO RETRIEVE AND UPDATE DATA.
/*          	
	var ref = new Firebase("https://resplendent-heat-2011.firebaseio.com/posts/");
